...
from modules.settings import BNB_RPC, DISABLE_SSL, OPBNB_RPC, REF_LINK, USER_IDS_TO_FOLLOW
from modules.config import DAILY_CLAIM_ABI
import aiohttp
...
class Account:
    def __init__(self, id: int, key: str, proxy: str, user_agent: str):
        ...
        self.quests_mapping = {
            ...
            # "GM": self.good_morning,
            # "Invite": self.invite_10,
            # "Explore": self.explore_user,
        }
...
################完善其他任务###################################
    ...
    # 每日任务——邀请10个人进自己的空间（即使是使用自己的ID访问空间10次，依旧可以完成）
    @retry
    async def invite_10():
        return 

    # 每日任务——探索5个不同的空间（疑似访问自己的空间5次也可以）
    @retry
    async def explore_user():
        return